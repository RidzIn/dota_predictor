...
def get_nn_pred(winrates, model, pick_1, pick_2):
    ...
    return {'pick_1': round(pred[0][0], 2),
            'pick_2': round(pred[0][1], 2)}


def get_row_prediction(winrates, model, pick_1, pick_2):
    """Return models prediction without any additional  checkers"""
    return {'simple': get_simple_pred(winrates, pick_1, pick_2),
            'xgb': get_nn_pred(winrates, model, pick_1, pick_2)}


def get_feedback_prediction(winrates, simple_feedback, xgb_feedback, model, pick_1, pick_2):
    ...
    for m in prediction.keys():
        ...
        result[m] = {'predicted_winrate': round(predicted_winrate / 5, 2),
...
def get_prediction(pick_1, pick_2):
    ...
    if row_prediction['simple']['pick_1'] > 0.50 > row_prediction['xgb']['pick_1'] or \
    ...
    predicted_pick_str = 'pick_1' if row_prediction['simple']['pick_1'] > 0.50 else 'pick_2'
    unpredicted_pick_str = 'pick_2' if row_prediction['simple']['pick_1'] > 0.50 else 'pick_1'

    if row_prediction['simple']['pick_1'] >= hyper_params['simple_row_threshold'] or \
            row_prediction['simple']['pick_2'] >= hyper_params['simple_row_threshold']:
        scores += 1

    if row_prediction['xgb']['pick_1'] >= hyper_params['xgb_row_threshold'] or \
            row_prediction['xgb']['pick_2'] >= hyper_params['xgb_row_threshold']:
        scores += 1

    feedback_prediction = get_feedback_prediction(winrates, simple_feedback, xgb_feedback, model, pick_1, pick_2)

    if feedback_prediction['simple']['predicted_winrate'] >= hyper_params['predicted_feedback_threshold']:
        scores += 1

    if feedback_prediction['simple']['unpredicted_winrate'] <= hyper_params['unpredicted_feedback_threshold']:
        scores += 1

    if feedback_prediction['xgb']['predicted_winrate'] >= hyper_params['predicted_feedback_threshold']:
        scores += 1

    if feedback_prediction['xgb']['unpredicted_winrate'] <= hyper_params['unpredicted_feedback_threshold']:
        scores += 1
