...
def get_matches_hero_appears(df, hero):
    ...
    condition = df['TEAM_0_HEROES'].apply(lambda x: hero in x) | df['TEAM_1_HEROES'].apply(lambda x: hero in x)
    return df[condition]


def get_matches_hero_with_hero(df, hero_1, hero_2):
    """Return matches where 2 heroes in the same pick, uses reshaped DataFrame"""
    condition = df['TEAM_0_HEROES'].apply(lambda x: hero_1 in x and hero_2 in x) | \
                df['TEAM_1_HEROES'].apply(lambda x: hero_1 in x and hero_2 in x)
    return df[condition]
...
def get_matches_hero_against_hero(df, hero_1, hero_2):
    ...
    condition = df['TEAM_0_HEROES'].apply(lambda x: hero_1 in x and hero_2 not in x) & \
                df['TEAM_1_HEROES'].apply(lambda x: hero_2 in x and hero_1 not in x) | \
                df['TEAM_0_HEROES'].apply(lambda x: hero_2 in x and hero_1 not in x) & \
                df['TEAM_1_HEROES'].apply(lambda x: hero_1 in x and hero_2 not in x)
    return df[condition]
...
def get_hero_stat(df, hero_to_calculate, hero_to_filter=None, against=True):
    ...
    return {'total': hero_total, 'wins': hero_wins, 'loses': hero_total - hero_wins, 'winrate': winrate}
...
def get_full_hero_stat(df, hero_to_calculate):
    ...
    for hero in heroes:
        if hero == hero_to_calculate:
            ...
            result[hero_to_calculate][hero]['against_winrate'] = \
            reverse_stat(get_hero_stat(df, hero_to_calculate, against=True))['winrate']
            result[hero_to_calculate][hero]['with_winrate'] = get_hero_stat(df, hero_to_calculate, against=False)[
                'winrate']
        else:
            result[hero_to_calculate][hero] = {'against_winrate': 0, 'with_winrate': 0}
            result[hero_to_calculate][hero]['against_winrate'] = \
            get_hero_stat(df, hero_to_calculate, hero, against=True)['winrate']
            result[hero_to_calculate][hero]['with_winrate'] = get_hero_stat(df, hero_to_calculate, hero, against=False)[
                'winrate']
